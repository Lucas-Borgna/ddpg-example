...
class TrackEnv:
    ...
    def step(self, a):
        ...
        distance = np.sqrt(
            (new_hit.r - next_hit.r) ** 2 + (new_hit.z - next_hit.z) ** 2
        )
        reward = -distance
    ...
    def reset(self):
        ...
        self.initial_event = pd.read_hdf(
            config["input_dir"]
            + config["file_name"]
        ...
        self.event = self.event[
            self.event["particle_id"].isin(np.array(nh[nh > 7].index))
        ]
        ...
        self.reset_count += 1
        self.original_particle = self.event[
            self.event["particle_id"] == self.original_pid
        ].reset_index()
        #
